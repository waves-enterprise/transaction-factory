...
import com.wavesenterprise.crypto.internals.{WavesAlgorithms, WavesPublicKey}
...
import com.wavesenterprise.lang.v1.evaluator.ctx.impl.{CryptoContext, EnvironmentFunctions, PureContext, _}
import com.wavesenterprise.lang.v1.testing.ScriptGen
import com.wavesenterprise.lang.v1.traits.Environment
import com.wavesenterprise.lang.v1.{CTX, FunctionHeader}
...
class EvaluatorV1Test extends PropSpec with ScalaCheckPropertyChecks with Matchers with ScriptGen with NoShrink {
  ...
  property("returns an success if sigVerify return a success") {
    ...
    val signature = global.sign(privateKey, bodyBytes)
  ...
  property("returns correct context") {
    val (alicePrivateKey, _)          = generateTestKeyPair()
    val (bobPrivateKey, bobPublicKey) = generateTestKeyPair()
    val (_, senderPublicKey)          = generateTestKeyPair()
    ...
    val (log, result) = multiSig(
      ...
      bobPublicKey,
      bobPublicKey,
      global.sign(alicePrivateKey, bodyBytes),
      global.sign(bobPrivateKey, bodyBytes)
    )
  ...
  property("returns an error if sigVerify return an error") {
    val (_, publicKey) = generateTestKeyPair()
    val bodyBytes      = "message".getBytes()

    val r = sigVerifyTest(bodyBytes, publicKey, "signature".getBytes())
  ...
  property("addressFromString works as the native one: wrong address version") {
    ...
    val gen = for {
      ...
      val (_, pk) = generateTestKeyPair()
      EnvironmentFunctions.AddressPrefix + Base58.encode(addressFromPublicKey(environment.chainId, pk.getEncoded, addressVersion))
    }
  ...
  property("addressFromString works as the native one: from other network") {
    ...
    val gen = for {
      ...
      val (_, pk) = generateTestKeyPair()
      EnvironmentFunctions.AddressPrefix + Base58.encode(addressFromPublicKey(chainId, pk.getEncoded))
    }
  ...
  private def sigVerifyTest(bodyBytes: Array[Byte], publicKey: WavesPublicKey, signature: Array[Byte]): Either[ExecutionError, Boolean] = {
    val txType = CaseType(
      "Transaction",
      List(
    ...
    val txObj = CaseObj(
      ...
      Map(
        ...
        "senderPk"  -> ByteVector(publicKey.getEncoded),
  ...
  private def multiSig(bodyBytes: Array[Byte],
                       senderPK: WavesPublicKey,
                       alicePK: WavesPublicKey,
                       bobPK: WavesPublicKey,
                       aliceProof: Array[Byte],
                       bobProof: Array[Byte]): (Log, Either[ExecutionError, Boolean]) = {
    val txType = CaseType(
      "Transaction",
      List(
        "bodyBytes" -> BYTEVECTOR,
    ...
    val txObj = CaseObj(
      ...
      Map(
        "bodyBytes" -> ByteVector(bodyBytes),
        "senderPk"  -> ByteVector(senderPK.getEncoded),
    ...
    val vars: Map[String, ((FINAL, String), LazyVal)] = Map(
      ...
      ("alicePubKey", ((BYTEVECTOR, "Alices test publik key"), LazyVal(EitherT.pure(ByteVector(alicePK.getEncoded))))),
      ("bobPubKey", ((BYTEVECTOR, "Bob test public key"), LazyVal(EitherT.pure(ByteVector(bobPK.getEncoded)))))
    )
