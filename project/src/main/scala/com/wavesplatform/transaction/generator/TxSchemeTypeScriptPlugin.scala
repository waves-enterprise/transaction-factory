...
import com.wavesplatform.transaction.generator.typescript.{TestsTypeScriptGenerator, TxTypeScriptGenerator}
...
object TxSchemeTypeScriptPlugin extends AutoPlugin {
  ...
  lazy val codeGenerator = Def.task {
    val srcPath  = (baseDirectory in Compile).value / "src"
    val testPath = (baseDirectory in Compile).value / "tests"
