...
import com.wavesplatform.transaction.assets.SetAssetScriptTransactionV1
...
class SetAssetScriptTransactionV1Specification extends GenericTransactionSpecification[SetAssetScriptTransactionV1] {
  def transactionParser: com.wavesplatform.transaction.TransactionParserFor[SetAssetScriptTransactionV1] = SetAssetScriptTransactionV1
  def updateProofs(tx: SetAssetScriptTransactionV1, p: Proofs): SetAssetScriptTransactionV1 = {
    tx.copy(proofs = p)
  }
  def generator: Gen[((Seq[com.wavesplatform.transaction.Transaction], SetAssetScriptTransactionV1))] = setAssetScriptTransactionGen
