...
class PolicyDataHashV1Specification extends FunSpecLike with ScalaCheckPropertyChecks with Matchers with TransactionGen with Inside {
  ...
  it("PolicyDataHash Transaction encoding round trip") {
    forAll(policyDataHashTransactionV1Gen()) {
      case PolicyDataWithTxV1(_, policyDataHashTx) =>
        ...
        assert(zeroByte === 0)
        assert(encodedTypeId === PolicyDataHashTransactionV1.typeId)
        assert(encodedVersion === 1)

        val decoded = PolicyDataHashTransactionV1.parseTail(version = 1, encoded, 3)
        decoded.fold(ex => fail(ex), decodedTx => policyDataHashTx shouldEqual decodedTx)
    }
  }

  it("PolicyDataHashTransactionV1 proto serialization roundtrip") {
    forAll(policyDataHashTransactionV1Gen()) {
      case PolicyDataWithTxV1(_, tx) =>
        val recovered = PolicyDataHashTransactionV1.fromProto(tx.toInnerProto).explicitGet()
    ...
    forAll(policyDataHashTransactionV1Gen()) {
      case PolicyDataWithTxV1(_, policyDataHashTx) =>
        ...
        TransactionParsers
          ...
          .fold(
            ex => fail(ex), {
              case tx: PolicyDataHashTransactionV1 =>
  ...
  it("JSON format validation") {
    ...
    val tx = PolicyDataHashTransactionV1(sender,
                                         policyDataHash,
                                         defaultPolicyId,
                                         timestamp,
                                         policyDataHashFee,
