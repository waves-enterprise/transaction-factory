...
import com.wavesplatform.acl.PermissionsGen
...
class UpdatePolicyTransactionV2Specification extends FunSpecLike with ScalaCheckPropertyChecks with Matchers with TransactionGen with Inside {

  private val defaultPolicyId = ByteStr(Base58.decode("thfgsdfewe").get)
  private val updatePolicyFee = TestFeeSettings.defaultFees.forTxType(CreatePolicyTransactionV2.typeId)

  it("UpdatePolicy Transaction encoding round trip") {
    forAll(updatePolicyTransactionV2Gen()) { updatePolicyTx =>
      ...
      val decoded = UpdatePolicyTransactionV2.parseTail(version = 1, encoded, 3)
      decoded.fold(ex => fail(ex), decodedTx => updatePolicyTx shouldEqual decodedTx)
    }
  }

  it("UpdatePolicyTransactionV2 proto serialization roundtrip") {
    forAll(updatePolicyTransactionV2Gen()) { tx =>
      val recovered = UpdatePolicyTransactionV2.fromProto(tx.toInnerProto).explicitGet()
      recovered shouldEqual tx
    }
  }

  it("serialize and deserialize UpdatePolicy Transaction with empty owners and recipients") {
    val updatePolicyTx = UpdatePolicyTransactionV2(
  ...
  it("UpdatePolicy Transaction's signature doesn't corrupt after encoding trip") {
    forAll(updatePolicyTransactionV2Gen()) { updatePolicyTx =>
      ...
      TransactionParsers
        ...
        .fold(
          ex => fail(ex), {
            case tx: UpdatePolicyTransactionV2 =>
  ...
  it("Success serialize and deserialize with empty lists") {
    ...
    val tx = UpdatePolicyTransactionV2(
      sender = sender,
      policyId = defaultPolicyId,
      recipients = List.empty,
      owners = List.empty,
