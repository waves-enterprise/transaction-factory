...
class CreatePolicyTransactionV2Specification extends FunSpecLike with ScalaCheckPropertyChecks with Matchers with TransactionGen with Inside {

  private val createPolicyFee = TestFeeSettings.defaultFees.forTxType(CreatePolicyTransaction.typeId)

  it("CreatePolicy Transaction encoding round trip") {
    forAll(createPolicyTransactionV2Gen()) {
      case CreatePolicyTransactionV2TestWrap(createPolicyTx, _) =>
        ...
        assert(zeroByte === 0)
        assert(encodedTypeId === CreatePolicyTransactionV2.typeId)
        assert(encodedVersion === 2)

        val decoded = CreatePolicyTransactionV2.parseTail(version = 2, encoded, 3)
        decoded.fold(ex => fail(ex), decodedTx => createPolicyTx shouldEqual decodedTx)
    }
  }

  it("CreatePolicyTransactionV2 proto serialization roundtrip") {
    forAll(createPolicyTransactionV2Gen()) {
      case CreatePolicyTransactionV2TestWrap(tx, _) =>
        val recovered = CreatePolicyTransactionV2.fromProto(tx.toInnerProto).explicitGet()
  ...
  it("Success serialize and deserialize with empty lists") {
    ...
    val tx = CreatePolicyTransactionV2(
      sender,
      policyName,
      description,
      List.empty,
    ...
    val decoded = CreatePolicyTransactionV2.parseTail(version = 1, encoded, 3)
  ...
  it("JSON format validation") {
    ...
    val tx = CreatePolicyTransactionV2(sender,
                                       policyName,
                                       description,
                                       recipients,
                                       owners,
