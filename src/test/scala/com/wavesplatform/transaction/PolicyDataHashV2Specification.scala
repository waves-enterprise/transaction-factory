...
class PolicyDataHashV2Specification extends FunSpecLike with ScalaCheckPropertyChecks with Matchers with TransactionGen with Inside {
  ...
  private val policyDataHashFee              = TestFeeSettings.defaultFees.forTxType(PolicyDataHashTransactionV2.typeId)
