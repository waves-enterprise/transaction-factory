...
class UpdatePolicyTransactionV3Specification extends FunSpecLike with ScalaCheckPropertyChecks with Matchers with TransactionGen with Inside {

  private val defaultPolicyId: ByteStr = ByteStr(Base58.decode("thfgsdfewe").get)
  private val updatePolicyFee          = TestFeeSettings.defaultFees.forTxType(CreatePolicyTransaction.typeId)
