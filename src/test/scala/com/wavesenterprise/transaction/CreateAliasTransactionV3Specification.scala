...
class CreateAliasTransactionV3Specification
    extends PropSpec
  ...
  property("CreateAliasTransaction serialization roundtrip") {
    forAll(createAliasV3Gen) { tx: CreateAliasTransaction =>
  ...
  property("CreateAliasTransaction proto serialization roundtrip") {
    forAll(createAliasV3Gen) { tx =>
      val recovered = CreateAliasTransactionV3.fromProto(tx.toInnerProto).explicitGet()
      recovered shouldEqual tx
    }
  }
  ...
  property("The same aliases from different senders have the same id") {
    forAll(accountGen, accountGen, aliasGen, timestampGen) {
      case (a1: PrivateKeyAccount, a2: PrivateKeyAccount, a: Alias, t: Long) =>
        val tx1 = CreateAliasTransactionV3.selfSigned(a1, a, MinIssueFee, t, None).explicitGet()
        val tx2 = CreateAliasTransactionV3.selfSigned(a2, a, MinIssueFee, t, None).explicitGet()
        tx1.id() shouldBe tx2.id()
    }
  }

  property("JSON format validation for CreateAliasTransactionV3") {
    val tx = CreateAliasTransactionV3
      .create(
