...
import com.wavesenterprise.crypto.internals.gost.KuznechikStream.Decryptor
...
object KuznechikStreamV2 {
  ...
  class Encryptor private(key: SecretKey, val chunkSize: Int = DefaultChunkSize) extends AbstractEncryptor(chunkSize) {
  ...
  class Decryptor private(key: SecretKey, val chunkSize: Int = DefaultChunkSize) extends AbstractDecryptor(chunkSize) {
    override protected def ivLength: Int = 16
    override protected def macLength: Int = 16
